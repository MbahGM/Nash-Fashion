...
ORDER_STATUSES = ["Paid", "Confirmed", "Packed", "Shipped", "Delivered", "RTO", "CR", "Cancelled"]
...
def get_product_price_display(product):
    ...
    if discounted < original:
        return f'<span class="text-gray-400 line-through text-sm">{original:,} XAF</span> <span class="text-rose-600 font-bold text-lg">{discounted:,} XAF</span>'
    return f'<span class="text-rose-600 font-bold text-lg">{original:,} XAF</span>'
...
def admin_header(active=""):
    links = [("Dashboard","/admin/dashboard"),("Products","/admin/products"),("Inventory","/admin/inventory"),("Orders","/admin/orders"),("Marketing","/admin/marketing"),("Reports","/admin/reports"),("Logout","/admin/logout")]
    nav = "".join([f'<a href="{u}" class="text-rose-100 mx-2 hover:text-white {"font-bold underline" if l==active else ""}">{l}</a>' for l,u in links])
...
def admin_marketing():
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
    <div class="bg-white rounded-lg shadow p-4"><h3 class="font-semibold mb-3">Hot Deals</h3><table class="w-full text-sm"><thead><tr class="border-b text-left"><th class="py-2">Product</th><th class="py-2">SKU</th><th class="py-2">Original</th><th class="py-2">Now</th><th class="py-2">Act</th></tr></thead><tbody>{deals_rows}</tbody></table></div>
    <div class="bg-white rounded-lg shadow p-4"><h3 class="font-semibold mb-3">Clearance</h3><table class="w-full text-sm"><thead><tr class="border-b text-left"><th class="py-2">Product</th><th class="py-2">SKU</th><th class="py-2">Price</th><th class="py-2">Act</th></tr></thead><tbody>{clearance_rows}</tbody></table></div></div>
    <div class="bg-white rounded-lg shadow p-4 mb-6"><h3 class="font-semibold mb-3">Discounted Products</h3><table class="w-full text-sm"><thead><tr class="border-b text-left"><th class="py-2">Product</th><th class="py-2">Discount</th><th class="py-2">Price</th><th class="py-2">Act</th></tr></thead><tbody>{disc_rows}</tbody></table></div>
    <div class="bg-white rounded-lg shadow p-4"><h3 class="font-semibold mb-3">Delivery Zones</h3><table class="w-full text-sm mb-3"><thead><tr class="border-b text-left"><th class="py-2">Zone</th><th class="py-2">Cost</th></tr></thead><tbody>{zones_rows}</tbody></table><p class="text-xs text-gray-500">Free delivery over <strong>{FREE_DELIVERY_THRESHOLD:,} XAF</strong>.</p></div>
    <div class="bg-yellow-50 border border-yellow-200 rounded p-4 mt-4"><h3 class="font-semibold text-sm mb-2">How to Manage</h3><ul class="text-sm text-gray-600 space-y-1"><li>Discounts: Edit a product, set Discount Type & Value</li><li>Deals/Clearance: Edit a product, check the tag boxes</li><li>Bulk: Use CSV Bulk Upload</li></ul></div></main></body></html>'''
# ─── Reports (abbreviated) ───
@app.route('/admin/marketing')
def admin_marketing():
    if not is_admin(): return redirect('/admin')
    return '<h1>Marketing Page Works!</h1><p><a href="/admin">Back to Admin</a></p>'
@app.route('/admin/reports')

@app.route('/admin/marketing')
def admin_marketing():
    if not is_admin(): return redirect('/admin')
    return '<h1>Marketing Works!</h1>'
@app.route('/admin/reports')
def admin_reports():
    if not is_admin(): return redirect('/admin')
    cards = ""
...
def report_sales():
    ...
    return f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Sales</title><script src="https://cdn.tailwindcss.com"></script></head><body class="bg-gray-50">{admin_header("Reports")}<main class="max-w-5xl mx-auto p-4"><a href="/admin/reports" class="text-rose-600 text-sm">&larr; Back</a><h2 class="text-xl font-semibold my-4">Sales Summary</h2><div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6"><div class="bg-white rounded-lg shadow p-4"><p class="text-sm text-gray-500">Today</p><p class="text-3xl font-bold text-rose-600">{tr:,} XAF</p><p class="text-sm text-gray-400">{len(today)} orders</p></div><div class="bg-white rounded-lg shadow p-4"><p class="text-sm text-gray-500">This Week</p><p class="text-3xl font-bold text-rose-600">{wr:,} XAF</p><p class="text-sm text-gray-400">{len(week)} orders</p></div><div class="bg-white rounded-lg shadow p-4"><p class="text-sm text-gray-500">This Month</p><p class="text-3xl font-bold text-rose-600">{mr:,} XAF</p><p class="text-sm text-gray-400">{len(month)} orders</p></div></div><div class="bg-white rounded-lg shadow p-4"><h3 class="font-semibold mb-3">All Orders</h3><table class="w-full text-sm"><thead><tr class="border-b text-left"><th class="py-2">Date</th><th class="py-2">Customer</th><th class="py-2">Total</th><th class="py-2">Zone</th><th class="py-2">Status</th></tr></thead><tbody>{rows or '<tr><td colspan="5" class="py-4 text-center text-gray-400">No orders</td></tr>'}</tbody></table></div></main></body></html>'''
...
def admin_orders():
    ...
    for idx, o in enumerate(reversed(orders)):
        ...
        rows += f'<tr class="border-b"><td class="py-3 px-4 text-xs">{o["ref"]}</td><td class="py-3 px-4">{o["name"]}</td><td class="py-3 px-4">{o["phone"]}</td><td class="py-3 px-4">{o["total"]:,}</td><td class="py-3 px-4">{o.get("delivery_zone","")}</td><td class="py-3 px-4"><span class="bg-{sc}-100 text-{sc}-700 px-2 py-1 rounded text-xs">{o["status"]}</span></td><td class="py-3 px-4 text-xs">{o["date"]}</td><td class="py-3 px-4"><a href="/admin/orders/{ri}" class="text-rose-600 text-xs">View</a></td></tr>'
